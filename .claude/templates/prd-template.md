# PRD: {Feature/Epic Name}

## Document Info
- **Version:** 1.0
- **Created:** {date}
- **Author:** PM Agent
- **Status:** Draft | Review | Approved
- **Discovery Ref:** @docs/0-DISCOVERY/PROJECT-UNDERSTANDING.md

---

## Problem Statement

### Current State
{How things work today}

### Pain Points
1. {Pain point 1}
2. {Pain point 2}

### Impact
{Quantified impact — cost, time, user frustration}

---

## Goals & Success Metrics

| Goal | Metric | Target | Measurement Method |
|------|--------|--------|-------------------|
| {Goal 1} | {KPI} | {Target value} | {How to measure} |
| {Goal 2} | {KPI} | {Target value} | {How to measure} |

---

## Target Users

### Primary Persona: {Name}
- **Role:** {job/role}
- **Goals:** {what they want to achieve}
- **Pain Points:** {current frustrations}
- **Tech Savviness:** Low | Medium | High

### Secondary Persona: {Name}
{Same format}

---

## Scope

### In Scope (MVP)
| Item | Description | Priority |
|------|-------------|----------|
| {Feature 1} | {brief description} | Must Have |
| {Feature 2} | {brief description} | Should Have |

### Out of Scope
| Item | Reason |
|------|--------|
| {Excluded 1} | {why excluded} |
| {Excluded 2} | {why excluded} |

### Future Considerations (v2+)
- {Future feature 1}
- {Future feature 2}

---

## Requirements

### Functional Requirements

| ID | Requirement | Priority | Acceptance Criteria |
|----|-------------|----------|---------------------|
| FR-01 | {description} | Must Have | {testable criteria} |
| FR-02 | {description} | Must Have | {testable criteria} |
| FR-03 | {description} | Should Have | {testable criteria} |

### Non-Functional Requirements

| ID | Category | Requirement | Target |
|----|----------|-------------|--------|
| NFR-01 | Performance | {requirement} | {measurable target} |
| NFR-02 | Security | {requirement} | {measurable target} |
| NFR-03 | Usability | {requirement} | {measurable target} |

---

## User Stories (High-Level)

| ID | Story | Priority |
|----|-------|----------|
| US-01 | As a {user}, I want {action} so that {benefit} | Must Have |
| US-02 | As a {user}, I want {action} so that {benefit} | Should Have |

---

## Constraints

- **Technical:** {constraint}
- **Business:** {constraint}
- **Timeline:** {constraint}
- **Budget:** {constraint}

---

## Assumptions

| Assumption | Impact if Wrong | Validation Plan |
|------------|-----------------|-----------------|
| {assumption 1} | {impact} | {how to validate} |
| {assumption 2} | {impact} | {how to validate} |

---

## Dependencies

| Dependency | Type | Owner | Status | Risk |
|------------|------|-------|--------|------|
| {dependency} | Internal/External | {team} | {status} | H/M/L |

---

## Risks & Mitigations

| Risk | Impact | Probability | Mitigation |
|------|--------|-------------|------------|
| {risk 1} | H/M/L | H/M/L | {mitigation} |
| {risk 2} | H/M/L | H/M/L | {mitigation} |

---

## Timeline

| Milestone | Target Date | Dependencies |
|-----------|-------------|--------------|
| PRD Approval | {date} | - |
| Design Complete | {date} | PRD Approval |
| MVP Release | {date} | Development |

---

## Open Questions

- [ ] {Question requiring stakeholder input}
- [ ] {Question requiring research}

---

## Appendix

### Discovery Reference
- @docs/0-DISCOVERY/PROJECT-UNDERSTANDING.md

### Related Documents
- {links}
